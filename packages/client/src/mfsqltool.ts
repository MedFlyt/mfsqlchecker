        ...
        for (const row of queryResult.rows) {
            for (const field of queryResult.fields) {
                ...
                try {
                    ...
                    throw new Error(`Error parsing column "${fieldName}" containing value "${oldVal}": ${String(err)}`);
                }
            }
        }
    ...
    export async function loadMigrationFiles(migrationsDir: string): Promise<AvailableMigration[]> {
        ...
        try {
            ...
            throw new MigrationError(
                ...
                `Error listing directory contents of '${migrationsDir}': ${String(err)}`);
        }
        ...
        for (const matchingFile of matchingFiles) {
            ...
            try {
                ...
                throw new MigrationError(
                    ...
                    `Error reading file: '${fileName}': ${String(err)}`);
            }
